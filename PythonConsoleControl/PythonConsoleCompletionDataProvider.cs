        ...
        public void GenerateDescription(string stub, string item, DescriptionUpdateDelegate updateDescription, bool isInstance)
        {
            ...
            if (!String.IsNullOrEmpty(item))
            {
                try
                {
                    ...
                    //TODO: в некоторых местах выплёвывает исключение о недоработке API (NOT_IMPLEMENTED)
                    object value = "";
                    AutocompletionInProgress = false;
                    try
                    {
                        value = commandLine.ScriptScope.Engine.CreateScriptSourceFromString(docCommand, SourceCodeKind.Expression).Execute(commandLine.ScriptScope);
                    }
                    catch
                    {
                        AutocompletionInProgress = false;
