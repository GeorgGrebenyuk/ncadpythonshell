...
* Неверная интерпретация enum: например, частая конструкция `OpenMode.ForRead` и т.д. Может быть решена вводом вспомогательного класса, например, AuxiliaryCommands из CADCommands.То есть вводится следующее соответствие:
